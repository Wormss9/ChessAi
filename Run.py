...
def transformall(moves):
    ...
    for move in clean_moves:
        if x % 2 == 0:
            ...
            transform2(transformtoarray(board1),
                       transformtoarray(pgnConverter.getFullFen()))
        x = x+1
        #print("Move: "+move)
...
def translate(path: str, white_won: bool):
    ...
    for filename in glob.glob(os.path.join(path, '*.pgn')):
        with open(os.path.join(os.getcwd(), filename), 'r') as file:
            ...
            print("File "+str(y)+"/1303 "+filename+". Last file in " + str(math.floor(
                duration/60))+" minutes " + str(round(duration % 60, 2)) + " seconds.")
            opened = time.time()
            line = file.readline()
            while line:
                line = line.replace('\n', '')
...
def save(board, move):
    ...
    global fileb
    global filem
    fileb.write((str(board)[1:-1].replace("\n", "").replace(".",
                ".0,").replace(" ", "")+"\n").replace(",\n", "\n"))
    filem.write((str(move)[1:-1]+"\n").replace(" ", ""))
...
with open('data/board'+filenumber+'.csv', 'a') as fileb, open('data/moves'+filenumber+'.csv', 'a') as filem:
    cProfile.runctx('translate("PGN/", True)',globals(),locals())
print("Translated in %.0f" % ((time.time()-start)/60) +
      " minutes and %.2f" % ((time.time()-start) % 60)+" seconds")
